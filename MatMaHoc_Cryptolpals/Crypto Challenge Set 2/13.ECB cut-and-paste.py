...
List_pad = ["\x01", "\x02", "\x03", "\x04", "\x05", "\x06", "\x07", "\x08", "\x09", "\x0A", "\x0B", "\x0C", "\x0D",
            "\x0E", "\x0F"]
...
def Decrypt_AES_ECB(input):
    ...
    index_padding = Check_padding_oracle(pt)    #unpading
    plaintext = pt[:index_padding]
    return plaintext


def encrypt_profile(input):
    str1 = profile_for(input)       #Chuyển input theo định dạng
    str2 = Encryt_AES_ECB(str1)     #Mã hoá nó
    return str2


def decrypt_profile(input):
    str1 = Decrypt_AES_ECB(input)   #Giải mã input
    dic1 = Convert_Str_to_Dic(str1) #Chuyển nó thành dictionary
    return dic1
